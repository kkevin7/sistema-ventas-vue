...
async function aumentarStock(id_acticulo, cantidad){
    ...
    let nStock = parseInt(stock)+parseIntI(cantidad);
...
export default {
    ...
    list: async (req, res, next) => {
        try {
            ...
            .find({ $or: [{ 'num_comprobante': new RegExp(valor, 'i') }, { 'serie_comprobantre': new RegExp(valor, 'i') }] }, { createAt: 0 })
            .populate('usuario', {nombre: 1})
            .pupulate('persona', {nombre: 1})
            .sort({ 'createAt': -1 });
            res.status(200).json(reg);
